...
contract RPSTest is DSTest {
    ...
    uint256[8][4] moveProofUints = [
        ...
        [
            0x1dc1522a88ecc6178a12eb5c939ef1134a85a938ae072ad95d3c031fa3bc96ab,
            0x29f4308bb7e9b2f08250fa61304b3ad764412a82a6c72ca3291d6403a808e53a,
            0x21215c93d1ea46eae241341d8743a9e0dc4f39c01ed2e6bd92900f88810392a,
            0x2f15cb0d935e82332c765f33d70af6a8e38f21503fa3099ef203e4328a814aa6,
            0x78ac23d4f51c384a6b216535db88ac7b7985edff3ffa102a85ddc8c869a2d54,
            0x2110c3c4d298452d224229c76221664c0ff0baac6a96bf34e2dcf3db5089c39b,
            0x16406cbd62a0cdce71c022db17373bfde3bae0d37b1fada8e7c618b8e22aa5be,
            0x1f835c22c7a2d5b69ed64ce6cdab1de8f45126160787f006b207b5516cc612d3
        ],
        [
    ...
    uint256[8][4] revealProofUints = [
        ...
        [
            0xcf044036f42b0553f29349c5c19b3c2cbeb1f29806644b12e1dd02321f09dc1,
            0x20b8cd84584d7a63cfa519ad2fd870a46b636048cfa2de0f3aca6d492b09ed17,
            0xa6bb08d54b0ae3df778a2f57928c1810979b5616e3fd97fc217ffa4477b1d49,
            0xf24e9ca3cea7a6a66d413f75883d5d712bf66fb253c66fbb2a056e6170e3912,
            0x665003a322790fd316a378c3db57012b347e57c01633c112937e0c9f417e651,
            0x2eb1e732c450fee65899c9d8a01cd6b3310609758be45686e72fc277a57bc745,
            0xa3a2635994ab4166f405018bbf08c268860d00dcb4bf84c5559ed098b07a32d,
            0x5d1ddcc01867082da472b48d0941f16e2ede6cb3a2afafee5aacc4e60673768
        ],
        [
    ...
    function generatePermitHash(address owner, address spender, uint256 value, uint256 nonce, uint256 deadline)
        public
        view
        returns (bytes32)
    {
    ...
    function testStartRound() public {
        ...
        rps.startRound(RPS.StartParams(proof, moveAttestations[0], 0xb07dad, 0, 0, 0, 0, "", ""));

        RPS.Round memory round = rps.getRound(roundBefore);
        assertEq(round.player1, address(this));
        assertEq(round.player2, address(0));
    ...
    function testSubmitMove2() public {
        ...
        rps.startRound(RPS.StartParams(proof, moveAttestations[0], 0, 0, 0, 0, 0, "", ""));

        vm.prank(someAddress);
        rps.submitMove2(RPS.Move2Params(roundId, move, 0, 0, "", ""));

        RPS.Round memory round = rps.getRound(roundId);
        assertEq(round.player2, someAddress);
        assertEq(round.winner, 0);
        assertEq(round.move1, rps.DEAD_MOVE());
    ...
    function testEndRound() public {
        ...
        uint256 roundId = rps.totalRounds();
        vm.prank(someAddress);
        rps.startRound(RPS.StartParams(startProof, moveAttestations[0], 0, 0, 0, 0, 0, "", ""));

        vm.prank(otherAddress);
        rps.submitMove2(RPS.Move2Params(roundId, move, 0, 0, "", ""));

        vm.prank(someAddress);
        rps.endRound(RPS.EndParams(revealProof, roundId, 0));

        uint64 bt = uint64(block.timestamp);

        RPS.Round memory round = rps.getRound(roundId);
        assertEq(round.player1, someAddress);
        assertEq(round.player2, otherAddress);
    ...
    function testAllWinnerCombos() public {
        for (uint8 i = 0; i < 3; i++) {
            for (uint8 j = 0; j < 3; j++) {
                ...
                uint256 roundId = rps.totalRounds();
                rps.startRound(RPS.StartParams(startProof, moveAttestations[i], 0, 0, 0, 0, 0, "", ""));
                rps.submitMove2(RPS.Move2Params(roundId, j, 0, 0, "", ""));
                rps.endRound(RPS.EndParams(revealProof, roundId, i));
    ...
    function testSubmitMove2Twice() public {
        ...
        uint256 roundId = rps.totalRounds();
        rps.startRound(RPS.StartParams(startProof, moveAttestations[0], 0, 0, 0, 0, 0, "", ""));
        rps.submitMove2(RPS.Move2Params(roundId, move1, 0, 0, "", ""));

        vm.expectRevert(getErrorBytes("ErrorRoundHasMove()"));
        rps.submitMove2(RPS.Move2Params(roundId, move2, 0, 0, "", ""));
    }
    ...
    function testWagerPayout() public {
        ...
        RPS.StartParams memory startParams;
        {
            ...
            startParams = RPS.StartParams(moveProofUints[0], moveAttestations[0], 0, 1, amount, deadline, v, r, s);
        }
        ...
        RPS.Move2Params memory move2Params;
        {
            ...
            move2Params = RPS.Move2Params(roundId, move2, deadline, v, r, s);
        }
        ...
        vm.prank(otherAddress);
        rps.endRound(RPS.EndParams(revealProofUints[0], roundId, move1));
    ...
    function testCollectForfeit() public {
        ...
        RPS.StartParams memory startParams;
        {
            ...
            (uint8 v, bytes32 r, bytes32 s) = vm.sign(somePrivateKey, hash);
            startParams = RPS.StartParams(moveProofUints[0], moveAttestations[0], 0, 2, amount, deadline, v, r, s);
        }
